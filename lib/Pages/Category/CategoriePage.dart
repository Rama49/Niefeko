  ...
  if (user != null) {
    ...
    CollectionReference userCart = FirebaseFirestore.instance.collection('Panier').doc(userID).collection('Items');
    DocumentSnapshot productSnapshot = await userCart.doc(productName).get();

    if (productSnapshot.exists) {
      // Le produit existe déjà dans le panier de l'utilisateur, augmenter simplement la quantité
      userCart.doc(productName).update({'quantity': FieldValue.increment(1)});
    } else {
      // Le produit n'existe pas encore dans le panier de l'utilisateur, l'ajouter
      userCart.doc(productName).set({
        'imageUrl': imageUrl,
        'name': productName,
        'price': price,
        'quantity': 1,
    ...
    // Vous pouvez afficher un message d'erreur ou rediriger vers la page de connexion
  }
}
